...
class Scraper:
    def __init__(self, args, champion, champ_id=0, content_tags=None):
        ...
        self.site = (
            "http://leagueoflegends.wikia.com/wiki/"
            + self.champion.replace(" ", "_")
            + "/LoL/Audio"
        )
        ...
        ) = self.quote_id_modified = self.found_classic_skin_quote = False
        self.h2 = (
            self.possible_champ_converse
        ) = self.prev_valid_quote_id = self.quote_id = self.renamed_quote_id = ""
    ...
    def handle_div(self, tag):
        if not self.found_classic and tag.get("title") in {
            "Classic",
            "Live",
            self.champion,
        }:
    ...
    def handle_a(self, tag):
        if not self.blacklisted_h2 and tag.get("href") is not None:
            a_href = tag.get("href")
            ...
            if (
                not self.found_classic_skin_quote
                and ogg_file_name
                and self.quote_by_correct_champ
            ):
                for skin in self.data_skin_blacklist:
                    # Check if blacklisted skin matches the audio file name.
                    if re.search(
                        skin.replace(" ", ""), ogg_file_name.group(0), re.IGNORECASE
                    ):
    ...
    def handle_li(self, tag):
        ...
        if self.possible_champ_converse and not self.blacklisted_h2:
            ...
            if pre_quote:
                ...
                if re.search(self.possible_champ_converse, pre_quote) and not re.search(
                    self.champion, pre_quote
                ):
                    self.quote_by_correct_champ = False
                else:
                    self.quote_by_correct_champ = True
    ...
    def handle_i(self, tag):
        if (
            not self.blacklisted_h2
            and self.quote_by_correct_champ
            and (
                self.found_classic_skin_quote
                or self.quote_id == self.prev_valid_quote_id
            )
        ):
            ...
            if quote:
                ...
                if not self.h2:
                    if re.search(r"select", self.quote_id, re.IGNORECASE):
                        ...
                        self.h2 = "(missing_h2." + str(self.missing_h2_count) + ")"
                ...
                if self.quote_id_modified:
                    ...
                    (
                        self.base_dict[self.champion]["quotes"][self.h2][self.quote_id]
                    ) = quote
            else:
...
class InputParser:
    ...
    def format_input(self, input_):
        with suppress(IndexError):
            ...
            self.f_input = re.sub("['].", lambda p: p.group(0).upper(), self.f_input)
            self.f_input = re.sub("[ ].", lambda p: p.group(0).upper(), self.f_input)
        return self.f_input
    ...
    def parse_multi(self):
        ...
        self.f_input_list = [self.format_input(input_) for input_ in self.input_list]
...
def id_lookup(champion):
    try:
        ...
        print("Lookup file 'name_id_dict.json' not found,", "setting champion ID to 0.")
        return 0
    champ_id = next(
        (
            name_id_dict["data"][champ]["id"]
            for champ in name_id_dict["data"]
            if champion == name_id_dict["data"][champ]["name"]
        ),
        0,
    )
    return champ_id
...
def update_name_id_dict(prompted=False):
    ...
    print(
        "Local version:\t{}".format(
            versions["local"] if versions["local"] is not None else "(Unable to check)"
        )
    )
    print(
        "Live version:\t{}".format(
            versions["live"] if versions["live"] is not None else "(Unable to check)"
        )
    )
    ...
    if versions["local"] != versions["live"] and versions["live"] is not None:
        if not prompted:
            input_ = input(
                "New patch available. Download new champion list?"
...
def main_multi(ip):
    print(
        "Enter a list of champions, separated by commas.",
    ...
    if ip.f_input_list:
        ip.champion_list = [ip.verify_champion(f_input) for f_input in ip.f_input_list]
        with suppress(TypeError):
...
def main_all():
    ...
    for champ in name_id_dict["data"]:
        ...
        if not scrape.get_content_tags():
            print(f"No quotes found for {champion}")
        else:
            scrape.populate_dictionary()
        # If a skin selector tab is found, content_tags is switched to the
        # Classic / Live / (Champion Name) tag, found_classic is set to True,
...
def main():
    ...
    if args.a and args.m:
        print(
            "Both Scrape Multi (-m) and Scrape All (-a) flags were included."
            "\nChoose one."
        )
        sys.exit()
    if args.d and not hasFile["RiotAPIData.py"]:
        print(
            "RiotAPIData.py unable to import. Downloading with the -d flag"
            " is disabled."
        )
    if (args.d or args.a) and not hasFile["riot_api_key.json"]:
        print(
            "Missing file 'riot_api_key.json'."
    ...
    if args.a and not hasFile["name_id_dict.json"] and not args.d:
        print(
            "No file 'name_id_dict.json' found. Downloading this dictionary"
            " of all champions is required to scrape all champion pages."
        )
        if hasFile["RiotAPIData.py"] and hasFile["riot_api_key.json"]:
            input_ = input("Download new dictionary of champions?" " | 'y' for yes ")
            if input_.lower() == "y":
    ...
    if args.i and not hasFile["name_id_dict.json"]:
        print(
            "No file 'name_id_dict.json' found. Unable to lookup champion"
            " IDs. Using 0 instead."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-m", help="Download quotes for multiple champions.", action="store_true"
    )
    parser.add_argument(
        "-a", help="Download quotes for all champions.", action="store_true"
    )
    parser.add_argument(
        "-d", help="Download updated list of champions.", action="store_true"
    )
    parser.add_argument(
        "-i",
        help="Lookup champion ID, otherwise store 0 for" " champion ID.",
        action="store_true",
    )
    parser.add_argument(
        "-e", help="Print exceptions for error reporting.", action="store_true"
    )
